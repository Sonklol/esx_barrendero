...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('barrendero:GiveItem')
		StartNPCJob()
		Done = true
	end
...
AddEventHandler('barrendero:hasEnteredMarker', function(zone)
...
AddEventHandler('barrendero:hasExitedMarker', function(zone)

	if zone == 'Sale' then
		TriggerServerEvent('barrendero:stopSale')
	end
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('barrendero:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('barrendero:hasExitedMarker', LastZone)
			end
		end
	end
end)
				...
				if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
					...
					if PlayerData.job.name == Config.nameJob then
						...
						if CurrentAction == 'Sale' then
							TriggerServerEvent('barrendero:startSale')
						end
